PrintPlotChrono = function (crn, rwl = NULL, file.name = "crono", crono.type = "RAW", 
    saveCronoJpg = TRUE) 
{
    ...
    crn[, 1:(ncol(crn) - 2)] <- round(crn[, 1:(ncol(crn) - 2)], 
        3)
    WriteMatrix(crn, File = FILE_NAME_CSV, sep = ";", row.names = TRUE, 
        row.name = "Year", ID = FALSE, col.width = 4)
    write.crn(CRN, paste(FILE_NAME, ".CRN", sep = ""))
    jpeg(paste(CRONO_NAME, ".jpg", sep = ""), width = 600, height = 600, 
        quality = 100)
    PlotChron(crn)
    dev.off()
    if (!is.null(rwl)) {
        jpeg(paste(CRONO_NAME, "-series.jpg", sep = ""), width = 600, 
            height = 600, quality = 100)
        yr.vec = as.numeric(rownames(rwl))
        matplot(yr.vec, rwl, ylim = c(0, max(rwl, na.rm = TRUE)), 
            type = "l", xlab = "Years", ylab = "", main = file.name, 
            las = 1, col = "grey20", lty = 1, lwd = 1)
