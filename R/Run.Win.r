Run.Win = function (rwl, start = FirstYear(rwl), winLength = 50, step = winLength/2, 
    stc = c(5, 2, 1), print = T) 
{
    ...
    colnames(runStat) <- c("start", "end", "tree", "core", "n.tot", 
        "n.wt", "n.bt", " r.tot", " r.wt", "  r.bt", " c.eff", 
        " r.eff", "   eps")
    ...
    runStat <- runStat[c(runStat[, 3] > 1), ]
    rownames(runStat) <- 1:nrow(runStat)
    if (print) {
        cat(rep("=", 94), "\n", sep = "")
        WriteMatrix(runStat, ID = T, ID.name = "Seq", row.names = F, 
            na = "-")
        cat(rep("=", 94), "\n", sep = "")
    }
    return(runStat)
