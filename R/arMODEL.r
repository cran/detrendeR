arMODEL = function (input = "", ...) 
{
    ...
    arMAX.lab <- tklabel(frame3.1, text = "AR model of max order:", 
        foreground = "blue")
    ...
    OnOk = function() {
        ...
        if (flag == TRUE) {
            eval(parse(text = paste("ArFunction(", tclvalue(filenamevar), 
                ", order.max=", as.numeric(tclvalue(arValue)), 
                ")", sep = "")))
            eval(parse(text = paste(tclvalue(tabnamevar), "<<-apply(", 
                tclvalue(filenamevar), ", 2,ar.func, order.max=", 
    ...
    cancel.but <- tkbutton(frame.exit, text = "    Cancel    ", 
        command = function() tkdestroy(tfARparent))
