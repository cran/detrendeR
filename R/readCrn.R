readCrn <- function(fname,
  ...
  if (is.null(header)) {
    ...
    yrcheck <- suppressWarnings(as.numeric(substr(
      hdr1, 7,
      10
    )))
    if (is.null(yrcheck) |
      length(yrcheck) != 1 | is.na(yrcheck) |
  ...
  if (is.head) {
    dat1 <-
      readLines(dat, n = n.header + 1)[-c(1:n.header)]
  } else {
  ...
  chron.stats <- read.fwf(fname,
    c(6, 4, 6, 6, 6, 7, 9, 9, 10),
    skip = skip.lines,
    strip.white = TRUE
  )
  dat <-
    read.fwf(fname, c(6, 4, rep(c(4, 3), 10)),
      skip = skip.lines,
      strip.white = TRUE
    )
  is.int <- function(x) {
  ...
  for (i in 1:nseries) {
    ...
    x <- dat[series.index == i, -c(1, 2, seq(
      from = 4, to = 22,
      by = 2
    ))]
    y <- dat[series.index == i, -c(1, 2, seq(
      from = 3, to = 21,
      by = 2
    ))]
    for (j in 1:nrow(x)) {
      ...
      for (k in 1:ncol(x)) {
        ...
        if (i == 1) {
          crn.mat[as.character(yr), ncol.crn.mat] <- y[
            j,
            k
          ]
        }
  ...
  crn.mat <-
    crn.mat[!apply(
      as.matrix(is.na(crn.mat[, -ncol.crn.mat])),
      1, all
    ), ]
