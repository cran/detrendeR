readTable = function () 
{
    ...
    choosefic <- function() {
        ...
        if (decSep == 2) 
            decsepch <- ","
        rdcom <- paste(tabname, " <<- read.table(file='", filename, 
            "', header=", varn, ", sep='", sepch, "', dec='", 
            decsepch, "')", sep = "")
        flagError <- try(eval(parse(text = rdcom)), silent = TRUE)
        if (class(flagError) == "try-error") {
            return(invisible(tk_messageBox(type = "ok", "Something went wrong!!!", 
                caption = "")))
        }
        tkdestroy(tf)
        rowNames.flag <- as.logical(tclObj(rowNames))
        if (rowNames.flag) {
            eval(parse(text = paste("rownames(", tabname, ")<<-", 
                tabname, "[,1]", sep = "")))
            eval(parse(text = paste(tabname, "<<-", tabname, 
                "[,-1]", sep = "")))
        }
    ...
    src <- tkscrollbar(frame.preview, repeatinterval = 5, command = function(...) tkyview(TXT, 
        ...))
    src1 <- tkscrollbar(frame.preview, repeatinterval = 5, orient = "horizontal", 
        command = function(...) tkxview(TXT, ...))
    TXT <- tktext(frame.preview, height = 5, width = 37, yscrollcommand = function(...) tkset(src, 
        ...), xscrollcommand = function(...) tkset(src1, ...), 
        background = "grey", wrap = "none")
    tkgrid(tklabel(frame.preview, text = "Preview:", foreground = "blue"), 
        sticky = "w")
    ...
    for (i in 1:NROW(temp)) {
        tkinsert(TXT, "end", paste(temp[i], "\n", sep = ""))
    }
    tkconfigure(TXT, state = "disabled")
    tkpack(frame.preview, fill = "x")
    frame2 <- tkframe(tf, relief = "groove")
    varnames.cbut <- tkcheckbutton(frame2, text = "Variables names on the first row of data file", 
        variable = varnames)
    tkgrid(varnames.cbut, columnspan = 3, sticky = "w")
    rownames.cbut <- tkcheckbutton(frame2, text = "Years on the first column of data file", 
        variable = rowNames)
    ...
    sep.entry <- tkentry(sepFrame, textvariable = otherSepVar, 
        width = 5)
    tkgrid(tklabel(sepFrame, text = "Field separator:", foreground = "blue"))
    tkgrid(tkradiobutton(sepFrame, text = "Default", value = 1, 
        variable = sepVar), sticky = "w")
    tkgrid(tkradiobutton(sepFrame, text = "Commas", value = 2, 
        variable = sepVar), sticky = "w")
    tkgrid(tkradiobutton(sepFrame, text = "Semicolon", value = 3, 
        variable = sepVar), sticky = "w")
    tkgrid(tkradiobutton(sepFrame, text = "Tab", value = 4, variable = sepVar), 
        sticky = "w")
    tkgrid(tkradiobutton(sepFrame, text = "Other", value = 5, 
        variable = sepVar), sep.entry, sticky = "w")
    decSepFrame <- tkframe(frame2, relief = "groove")
    tkgrid(tklabel(decSepFrame, text = "Decimal separator:", 
        foreground = "blue"))
    tkgrid(tkradiobutton(decSepFrame, text = "Period [.]", value = 1, 
        variable = decSepVar), sticky = "w")
    tkgrid(tkradiobutton(decSepFrame, text = "Comma [,]", value = 2, 
        variable = decSepVar), sticky = "w")
