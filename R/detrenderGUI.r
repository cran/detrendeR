detrenderGUI = function () 
{
    ...
    site.entry <- tkentry(frame1.1, textvariable = site.value, 
        width = 3)
    site.lab <- tklabel(frame1.1, text = "Site: ")
    tkpack(site.lab, site.entry, side = "left")
    tree.entry <- tkentry(frame1.2, textvariable = tree.value, 
        width = 3)
    tree.lab <- tklabel(frame1.2, text = "Tree:")
    tkpack(tree.lab, tree.entry, side = "left")
    core.entry <- tkentry(frame1.3, textvariable = core.value, 
        width = 3)
    ...
    tkgrid(tklabel(frame.1, text = "          - Select Series - ", 
        foreground = "blue"))
    select.series.cbut <- tkcheckbutton(frame.1, text = "Delete series shorter than:", 
        variable = remove.shorter.series.value)
    select.series.entry <- tkentry(frame.1, textvariable = delete.shorter.series.value, 
        width = 4)
    ...
    makeSegPlot.cbut <- tkcheckbutton(frame.1.1, text = "Make graph with length series", 
        variable = makeSegPlot.value)
    ...
    interactive.detrend.cbut <- tkcheckbutton(frame2, text = "Interactive detrending", 
        variable = interactive.detrend.value)
    tkpack(interactive.detrend.cbut, side = "bottom", anchor = "w")
    Det1frame <- tkwidget(frame2, "labelframe", foreground = "blue", 
        text = "First detrend: ", relief = "groove", borderwidth = 2)
    Det2frame <- tkwidget(frame2, "labelframe", foreground = "blue", 
        text = "Second detrend: ", relief = "groove", borderwidth = 2)
    ...
    frame3.1 <- tkwidget(frame3, "labelframe", foreground = "blue", 
        text = "Autorregresive model: ", relief = "groove", borderwidth = 2)
    makeAr.cbut <- tkcheckbutton(frame3.1, text = "Autorregresive model of order:", 
        variable = makeAr.value)
    arMAX.entry <- tkentry(frame3.1, textvariable = arMAX.value, 
        width = 2)
    tkgrid(makeAr.cbut, arMAX.entry)
    frame3.2 <- tkwidget(frame3, "labelframe", foreground = "blue", 
        text = "Mean: ", relief = "groove", borderwidth = 2)
    ...
    frame4.label <- tklabel(frame4, text = " - EPS analysis - ", 
        foreground = "blue")
    run.win.analysis.cbut <- tkcheckbutton(frame4.1.1, text = "EPS window analysis", 
        variable = run.win.analysis.value)
    tkpack(frame4.label, fill = "x")
    tkpack(run.win.analysis.cbut, fill = "x", side = "left")
    tkpack(frame4.1.1, fill = "x")
    winLength.entry <- tkentry(frame4.1.2, textvariable = winLength.value, 
        width = 3)
    ...
    stepWin.entry <- tkentry(frame4.1.3, textvariable = stepWin.value, 
        width = 3)
    stepWin.lab <- tklabel(frame4.1.3, text = "                  Lag: ")
    tkpack(stepWin.lab, stepWin.entry, side = "left")
    tkpack(frame4.1.3, fill = "x")
    make.common.EPS.cbut <- tkcheckbutton(frame4.2.1, text = "Common interval", 
        variable = make.common.EPS.value)
    tkpack(make.common.EPS.cbut, fill = "x", side = "left")
    make.select.period.EPS.cbut <- tkcheckbutton(frame4.2.2, 
        text = "Period        First year:", variable = make.select.period.EPS.value)
    first.year.common.entry <- tkentry(frame4.2.2, textvariable = first.year.common.value, 
        width = 4)
    tkpack(make.select.period.EPS.cbut, first.year.common.entry, 
        fill = "x", side = "left")
    last.year.common.entry <- tkentry(frame4.2.3, textvariable = last.year.common.value, 
        width = 4)
    ...
    OnOk = function() {
        ...
        if (method1 == "No Detrending") 
            .assign("makeFirstDetrending", FALSE)
        .assign("first.detrending.method", GetDetrendMethod(method1, 
            n1, nPerc1, p1))
        ...
        if (method2 == "No Detrending") 
            .assign("makeSecondDetrending", FALSE)
        .assign("second.detrending.method", GetDetrendMethod(method2, 
            n2, nPerc2, p2))
        ...
        stc. <- c(toNumber(tclvalue(site.value)), toNumber(tclvalue(tree.value)), 
            toNumber(tclvalue(core.value)))
        if (sum(stc.) != 8) {
    ...
    tkpack(Cancel.but, Ok.but, side = "left", expand = "TRUE", 
        fill = "x")
