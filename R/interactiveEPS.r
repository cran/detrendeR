    ...
    OnOk = function() {
        ...
        if (flag == TRUE) {
            ...
            if (run.win.analysis) {
                ...
                cat(paste("\nRunning analysis:\n", sep = ""), 
                  sep = "")
                eval(parse(text = paste(output, "<<-Run.Win(", 
                  input, ", winLength=", winLength, ", stc=stc,  step =", 
                  stepWin, ")", sep = "")))
                eval(parse(text = paste("for (i in c(0.75, 0.80, 0.85, 0.90)) {EPS.resume(", 
                  output, ", EPS=i)}", sep = "")))
            }
            if (as.logic(tclvalue(make.common.EPS.value))) {
                eval(parse(text = paste("EPS.common.interval(", 
                  input, ", stc=stc, out=FALSE)")))
            }
            if (as.logic(tclvalue(make.select.period.EPS.value))) {
                if (!(is.null(first.year.common) || is.null(last.year.common.value))) {
                  eval(parse(text = paste("EPS.common.interval(", 
                    input, ", first.year.common=", first.year.common, 
