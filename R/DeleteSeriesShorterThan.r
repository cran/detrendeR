DeleteSeriesShorterThan = function (rwl, filename, rwlStats = NULL, YEAR = 100, SaveShort = FALSE, 
    Rwl = TRUE) 
{
    ...
    if (is.null(rwlStats)) {
        ...
        series.stats$Year = series.stats$Last - series.stats$First + 
            1
    ...
    if (ncol(rwl1) == 0) {
        cat("All series are shorter than ", YEAR, ".\n", sep = "")
        SaveShort = FALSE
        Rwl = FALSE
    }
    if (Rwl) {
    ...
    }
    {
        if (nSeries != nSeries1) {
            ...
            if (Rwl) 
                cat(paste("All series are longer than ", YEAR, 
                  ".\n", sep = ""))
        }
    }
    cat(rep("=", 98), "\n", sep = "")
